...
Each profile can now use multiple chatmail relays or mail servers for sending and receiving messages.
...
and transport operators can sleep better knowing that their relay going down will not prevent users from chatting.
For Rust experts interested in the implementation
maybe start from the [the "multi-relay" development issue](https://github.com/chatmail/core/issues/7357).
...
We are setting ourselves a mad goal:
providing an unstoppable, planetary-scale and very private Internet messaging experience,
while evolving and improving on IETF Internet messaging (email) protocols,
...
or exploring the wilderness of using or creating [secure mini-apps](https://webxdc.org).
