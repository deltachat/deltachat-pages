...
which runs stably since the [December 2023 Chatmail announcement](https://delta.chat/en/2023-12-13-chatmail).  

In addition, there are community-run chatmail servers: 

- [mehl.cloud](https://mehl.cloud) is geared towards german speaking
  users and runs stably since January 2024. 

- [mailchat.pl](https://mailchat.pl) is geared towards polish speaking
  users and runs stably since January 2024. 

- [bcc.chat](https://bcc.chat) operated in the US and runs
  stably since February 2024. 
...
See [provider database](https://providers.delta.chat) for options. 
...
and typically only sees encrypted messages and 
...
which means that server operators can never read your messages even if they try,
...
All chatmail servers are automatically deployed and updated using the 
...
Chatmail servers happily run on low-end hardware like a Rasperry Pi. 
