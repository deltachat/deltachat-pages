...
- Criação instantânea de **perfis de chat privados **
...
- Suporte a [vários perfis](#multiple-accounts) e
  a [vários dispositivos](#multiclient) em todas as plataformas
...
### O que são perfis? Como posso alternar entre eles? {#multiple-accounts}

Um perfil é **um nome, uma foto** e algumas informações adicionais para criptografar mensagens.
Um perfil reside apenas em seu(s) dispositivo(s)
e usa o servidor somente para transportar mensagens.

Na primeira instalação do Delta Chat, é criado um primeiro perfil.
...
Para usar as funções, toque e segure ou clique com o botão direito do mouse em um conversa na lista conversas.


### Como funcionam as "Mensagens salvas"? {#save}

**Saved Messages** é um chat que você pode usar para lembrar e localizar facilmente as mensagens.
...
- **Um tique** <img style="vertical-align:middle; width:1.5em; margin:1px" src="../assets/help/tick1.png" alt=""/>
  significa que a mensagem foi enviada com sucesso para o [relay](#relays).
...
as mensagens (de qualquer forma criptografadas) podem demorar mais para serem excluídas do servidor deles.
...
No entanto, como os grupos são [destinados a pessoas de confiança](#groups), evite compartilhá-los publicamente.
...
mas o Delta Chat é um mensageiro privado para conversar com [direitos iguais](#groups).
