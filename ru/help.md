...
Ваши друзья могут поделиться вашим контактом с другими друзьями,
это отображается как <b style="border: 1px solid currentColor; padding: 0 3px; font-size:90%">Запрос</b>
...
По соображениям конфиденциальности, никто не увидит изображение вашего профиля, пока вы не напишете
им сообщение.
...
- **Отправить в архив** необходимо, если вы не хотите больше видеть их в списке чатов.
  ...
  и будут отмечены как <b style="border: 1px solid currentColor; padding: 0 3px; font-size:90%">Архивировано</b>
...
- Наконец, вы можете использовать "Сохраненные сообщения", для создания **личных заметок** - откройте чат, напечатайте что-нибудь, добавьте фото или голосовое сообщение и т.д.
...
- **Одна галочка** <img style="vertical-align:middle; width:1.5em; margin:1px" src="../assets/help/tick1.png" alt=""/>
  означает, что сообщение успешно отправлено на [релей](#relays).

- **Две галочки** <img style="vertical-align:middle; width:1.5em; margin:1px" src="../assets/help/tick2.png" alt=""/>
  указывают на то, что ваш контакт прочитал сообщение.

В [группах](#groups) вторая галочка означает, что хотя бы один из участников подтвердил прочтение сообщения.

Вторая галочка появится только в том случае, если у вас и хотя бы одного из получателей, прочитавшего сообщение,
включена опция **Настройки → Чаты → Уведомление о прочтении**.
...
### Как обеспечивается качество мультимедиа? {#mediaquality}

Изображения, видео, файлы, голосовые сообщения и т.д. можно отправлять с помощью кнопок <img style="vertical-align:middle; width:1.0em; margin:1px" src="../assets/help/paperclip.png" alt="Paperclip"/> **Вложение**
...
- Если вам необходимо отправить мультимедийный файл в **исходном качестве**, используйте значок <img style="vertical-align:middle; width:1.0em; margin:1px" src="../assets/help/paperclip.png" alt="Paperclip"/> **Прикрепить → Файл** в чате.
...
### Что произойдет, если я включу функцию "Удалять сообщения с устройства"? {#delold}

Если вы хотите сэкономить место на устройстве, можно выбрать функцию автоматического удаления старых
сообщений.

Чтобы включить эту функцию, перейдите в **Настройки → Чаты → Удалять сообщения с устройства**.
Вы можете установить временные рамки от "через 1 час" до "через 1 год";
Таким образом, *все* сообщения будут удаляться с вашего устройства, как только они станут
...
- На следующем экране выберите **участников группы** и придумайте **название группы**. Вы также можете выбрать **аватар группы**.
...
Все участники группы имеют **одинаковые права**.
Поэтому каждый может удалить любого участника или добавить нового.
...
Поскольку вы больше не являетесь участником группы, вы не можете добавить себя снова. 
...
Или, вместо этого, вы можете "отключить уведомления" для  группы - в этом случае вы будете получать все сообщения и 
можете их писать, но больше не будете получать уведомления о новых сообщениях.
...
Строгого технического ограничения нет,
но не рекомендуется создавать группы больше 150 участников.

По мере увеличения размера групп они могут стать социально нестабильными и потребовать иерархии, 
в то время как Delta Chat - это приватный мессенджер для общения на [равных правах](#groups).
...
- В **Windows** перейдите в "Панель управления / Сеть и Интернет"
  ...
  (после передачи можно вернуть исходное значение)
...
- На старом устройстве перейдите в **Настройки → Чаты → Экспорт резервной копии**. Введите свой
...
- На новом устройстве выберите **У меня уже есть профиль → Восстановить из резервной копии**.
  Если вы используете iOS: и у вас возникли трудности,
  возможно, [это руководство](https://support.delta.chat/t/import-backup-to-ios/1628) поможет вам.
...
- [Autocrypt v2](https://autocrypt2.org), полное внедрение которого запланировано на 2026 год, 
  обеспечит поддержку постквантового шифрования и прямой секретности.
...
- Адреса отправителя и получателя, по умолчанию генерируются случайным образом
- Размер сообщения
...
Используемый [релей](#relays) должен знать ваш IP-адрес,
а также иногда устройства ваших контактов, если вы проводите совместные [звонки](#experiments)
или используете [приложения](#webxdc).

IP-адреса необходимы для обеспечения соединения и эффективности.
Они не сохраняются и не передаются третьим лицам.
Обратите внимание, что IP-адрес
- это не подробный адрес, который вы указываете службе доставки,
а скорее приблизительный, обычно определяющий регион или страну.

Поскольку именно так по умолчанию работает интернет и другие мессенджеры,
мы не предлагаем здесь никаких настроек и не задаём предварительных вопросов

Если вы считаете свой IP-адрес угрозой безопасности или конфиденциальности,
...
Например, нажатие на ссылку раскрывает IP-адрес неизвестным лицам и представляет собой гораздо больший риск в данном случае.
...
Пока нет, но это будет реализовано в [Autocrypt v2](https://autocrypt2.org). 
...
Этот подход описан в черновике спецификации [Autocrypt v2 OpenPGP Certificates](https://datatracker.ietf.org/doc/draft-autocrypt-openpgp-v2-cert/). 

### Поддерживает ли Delta Chat Post-Quantum-Cryptography (Постквантовую криптографию)? {#pqc}

Пока нет, но эта возможность появится в [Autocrypt v2](https://autocrypt2.org).

[Autocrypt v2](https://autocrypt2.org), полное внедрение которого запланировано на 2026 год,
обеспечит поддержку постквантового шифрования для защиты от атак с использованием квантовых компьютеров.
Delta Chat использует Rust-библиотеку OpenPGP [rPGP](https://github.com/rpgp/rpgp)
которая поддерживает актуальный черновик IETF [IETF Post-Quantum-Cryptography OpenPGP](https://datatracker.ietf.org/doc/draft-ietf-openpgp-pqc/).
Особенности реализации описаны в черновике спецификации [Autocrypt v2 OpenPGP Certificates](https://datatracker.ietf.org/doc/draft-autocrypt-openpgp-v2-cert/). 
...
- В марте 2023 года компания [Cure53](https://cure53.de) провела анализ шифрования транспортного уровня сетевых соединений Delta Chat, а также проверку воспроизводимой конфигурации почтового сервера,
  [рекомендованной на этом сайте](https://delta.chat/serverguide).
  Подробнее об аудите можно узнать [в нашем блоге](https://delta.chat/en/2023-03-27-third-independent-security-audit),
  а [полный отчет доступен по этой ссылке](../assets/blog/MER-01-report.pdf).
...
- Фонд [NLnet Foundation](https://nlnet.nl/) выделил в 2019/2020 году 46 тысяч евро на
  доработку связки Rust/Python и создание экосистемы чат-ботов..
